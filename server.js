...
app.get("/", async (req, res) => {
  ...
  try {
    const result = await client.query(
      "SELECT users.username FROM sessions JOIN users ON sessions.user_id = users.id WHERE sessions.session_id = $1 AND sessions.expires_at > NOW()",
      [sessionId]
    );
...
app.post("/login", async (req, res) => {
  ...
  try {
    const q1_resp = await client.query(
      "SELECT id, username, password FROM users WHERE username = $1",
      [username]
    );
    if (q1_resp.rows.length === 0) {
      return res.send(
        `<script> alert("Incorrect Credentials."); window.location.href = "/login" </script>`
      );
    }
    ...
    await client.query(
      "INSERT INTO sessions (session_id, user_id, expires_at) VALUES ($1, $2, NOW() + interval '1 hour')",
      [sessionId, user.id]
    );
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    return res.send(
      `<script> alert("Account successfully Created!"); window.location.href = "/login" </script>`
    );
  } catch (err) {
    return res.send(
      '<script> alert("Username taken."); window.location.href = "/register" </script>'
    );
  }
});
...
app.post("/api/account/change_password", async (req, res) => {
  ...
  const result = await client.query(
    "SELECT users.username, users.password FROM sessions JOIN users on sessions.user_id = users.id WHERE sessions.session_id = $1 AND sessions.expires_at > NOW()",
    [sessionId]
  );
  if (result.rows.length === 0) {
  ...
  try {
    ...
    if (new_password_1 !== new_password_2) {
      return res.send(
        '<script> alert("Password do not match"); window.location.href="/account_settings"</script>'
      );
    } else if (
      new_password_1 === old_password ||
      new_password_2 === old_password
    ) {
      return res.send(
        '<script> alert("You cannot use the same password"); window.location.href="/account_settings"</script>'
      );
    }
    const saltRounds = 10;
    ...
    if (matching) {
      return res.send(
        '<script> alert("Passwords are the same"); window.location.href="/account_settings"</script>'
      );
    }
    if (!correct_old) {
      return res.send(
        '<script> alert("Incorrect password"); window.location.href="/account_settings"</script>'
      );
    }
    ...
    return res.send(
      '<script> alert("Password successfully updated"); window.location.href="/account_settings"</script>'
    );
  } catch (err) {
...
app.get("/api/modules/category/:category", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "No modules found for this category" });
    }
...
app.get("/api/modules/get_pinned", async (req, res) => {
  try {
    ...
    if (usr_result.rows.length === 0)
      return res.status(401).json({ erorr: "Unauthorised" });
    const user_id = usr_result.rows[0].id;
    const result = await client.query(
      "SELECT mdl.id, mdl.name, mdl.thumb, mdl.description, umd.pinned, COALESCE(umd.completed, false) AS completed FROM modules mdl LEFT JOIN user_module_data umd ON umd.module_id = mdl.id AND umd.user_id = $1 WHERE umd.pinned = true",
      [user_id]
    );
    res.json(result.rows);
  } catch (err) {
    return res.json({ error: "Failed to obtain pinned modules." });
  }
});
...
app.get("/api/modules/content", async (req, res) => {
  try {
    ...
    const user_id = usr_result.rows[0].id;
    const result = await client.query(
      "SELECT mdl.id, mdl.name, mdl.thumb, mdl.description, mdl.content, COALESCE(umd.completed, false) AS completed FROM modules mdl LEFT JOIN user_module_data umd ON umd.module_id = mdl.id AND umd.user_id = $1 WHERE mdl.category = 'cybersecurity'",
      [user_id]
    );
    res.json(result.rows);
  } catch (err) {
    return res.json({ error: "Failed to load module content" });
  }
});
...
app.get("/api/modules/:id/status", async (req, res) => {
  ...
  try {
    const result = await client.query(
      "SELECT completed FROM user_module_data WHERE user_id = $1 AND module_id = $2",
      [user_id, module_id]
    );
...
app.post("/api/modules/:id/pin", async (req, res) => {
  ...
  try {
    const check = await client.query(
      "SELECT pinned FROM user_module_data WHERE user_id = $1 AND module_id = $2",
      [user_id, module_id]
    );
    if (check.rows.length > 0) {
      const newly_pinned = !check.rows[0].pinned;
      await client.query(
        "UPDATE user_module_data SET pinned = $1 WHERE user_id = $2 AND module_id = $3",
        [newly_pinned, user_id, module_id]
      );
      return res.json({ pinned: newly_pinned });
    } else {
      await client.query(
        "INSERT INTO user_module_data (user_id, module_id, pinned) VALUES ($1, $2, true)",
        [user_id, module_id]
      );
...
app.post("/api/modules/:id/complete", async (req, res) => {
  ...
  try {
    ...
    if (check.rows.length > 0) {
      ...
      await client.query(
        "UPDATE user_module_data SET completed = $1 WHERE user_id = $2 AND module_id = $3",
        [updated_stat, user_id, module_id]
      );
    } else {
      updated_stat = true;
      await client.query(
        "INSERT INTO user_module_data (user_id, module_id, completed) VALUES ($1, $2, true)",
        [user_id, module_id]
      );
    }
...
app.post("/api/module/:id/delete", async (req, res) => {
  const sessionId = req.cookies.sessionId;
  if (!sessionId) return res.redirect("/login");
  const userResult = await client.query(
    "SELECT users.role FROM sessions JOIN users ON sessions.user_id = users.id WHERE sessions.session_id = $1 AND sessions.expires_at > NOW()",
    [sessionId]
  );
...
app.post("/api/modules/create", async (req, res) => {
  ...
  try {
    await client.query(
      "INSERT INTO modules (name, description, content, category) VALUES ($1, $2, $3, $4)",
      [name, description, content, category]
    );
    return res.json({ success: true, message: "Module successfully created" });
  } catch (err) {
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!sessionId) return res.status(401).json({ error: "Unauthorised" });
    const usr_result = await client.query(
      "SELECT users.id, users.avatar FROM sessions JOIN users ON sessions.user_id = users.id WHERE sessions.session_id = $1 AND sessions.expires_at > NOW()",
      [sessionId]
    );
    if (usr_result.rows.length === 0)
      return res.status(401).json({ erorr: "Unauthorised" });
...
app.get("/dashboard", async (req, res) => {
  ...
  if (!sessionId) return res.redirect("/login");
  const result = await client.query(
    "SELECT users.username, users.avatar FROM sessions JOIN users ON sessions.user_id = users.id WHERE sessions.session_id = $1 AND sessions.expires_at > NOW()",
    [sessionId]
  );
...
app.get("/account_settings", async (req, res) => {
  ...
  if (!sessionId) return res.redirect("/login");
  const result = await client.query(
    "SELECT users.username, users.avatar, users.role FROM sessions JOIN users ON sessions.user_id = users.id WHERE sessions.session_id = $1 AND sessions.expires_at > NOW()",
    [sessionId]
  );
  if (result.rows.length === 0) return res.redirect("/login");
  const username = result.rows[0].username;
  ...
  let account_settings_html = await fsp.readFile(
    path.join(process.cwd(), "public", "account_settings.html"),
    { encoding: "utf8" }
  );
  account_settings_html = account_settings_html
    .replace("{{username}}", username)
    .replace(/{{avatar}}/g, avatar)
    .replace(/{{role}}/g, role);
...
app.get("/admin/settings", async (req, res) => {
  ...
  const avatar = await getAvatar(result.rows[0].avatar);
  let admin_settings_html = await fsp.readFile(
    path.join(process.cwd(), "public", "admin_settings.html"),
    { encoding: "utf8" }
  );
...
app.get("/admin/settings/module_creation", async (req, res) => {
  const sessionId = req.cookies.sessionId;
  if (!sessionId) return res.redirect("/login");
  const result = await client.query(
    "SELECT users.username, users.avatar, users.role FROM sessions JOIN users ON sessions.user_id = users.id WHERE sessions.session_id = $1 AND sessions.expires_at > NOW()",
  ...
  if (result.rows[0].role !== "admin") return res.redirect("/dashboard");
  const avatar = await getAvatar(result.rows[0].avatar);
  let module_creation_html = await fsp.readFile(
    path.join(process.cwd(), "public", "module_creation.html"),
    { encoding: "utf8" }
  );
  module_creation_html = module_creation_html.replace(/{{avatar}}/g, avatar);
  res.send(module_creation_html);
});
...
app.get("/learn", async (req, res) => {
  ...
  let learn_html = await fsp.readFile(
    path.join(process.cwd(), "public", "learn.html"),
    { encoding: "utf8" }
  );
  learn_html = learn_html
    .replace("{{username}}", username)
    .replace(/{{avatar}}/g, avatar);
  res.send(learn_html);
});
...
app.get("/learning", async (req, res) => {
  ...
  const mdl_res = await client.query(
    "SELECT name, description, content FROM modules WHERE id = $1",
    [moduleId]
  );
  ...
  const avatar = await getAvatar(result.rows[0].avatar);
  let learning_html = await fsp.readFile(
    path.join(process.cwd(), "public", "learning.html"),
    { encoding: "utf8" }
  );
  learning_html = learning_html
    ...
    .replace(/{{avatar}}/g, avatar)
    .replace("{{module_id}}", String(moduleId));
